...
def nested_patches(img_slide, out_base, level=(0,), ext='jpeg'):
    ...
    n_levels = len(glob.glob('WSI_tmp_files/*'))
    bag_path = os.path.join(out_base, img_class, img_name)
    os.makedirs(bag_path, exist_ok=True)
    if len(level)==1:
        patches = glob.glob(os.path.join('WSI_tmp_files', '*', '*.'+ext))
        ...
        levels = [int(os.path.basename(i)) for i in glob.glob(os.path.join('WSI_tmp_files', '*'))]
        levels.sort()
        low_patches = glob.glob(os.path.join('WSI_tmp_files', str(levels[0]), '*.'+ext))
        for i, low_patch in enumerate(low_patches):
            ...
            for x_pos in high_x_list:
                for y_pos in high_y_list:
                    high_patch = glob.glob(os.path.join('WSI_tmp_files', str(levels[1]), '{}_{}.'.format(x_pos, y_pos)+ext))
                    if len(high_patch)!=0:
...
if __name__ == '__main__':
    ...
    all_slides = glob.glob(os.path.join(path_base, '0/*.'+args.slide_format)) +  glob.glob(os.path.join(path_base, '0/*/*.'+args.slide_format)) + glob.glob(os.path.join(path_base, '1/*.'+args.slide_format)) +  glob.glob(os.path.join(path_base, '1/*/*.'+args.slide_format)) 
    
    # pos-i_pos-j -> x, y
    for idx, c_slide in enumerate(all_slides):
        print('Process slide {}/{}'.format(idx+1, len(all_slides)))
        DeepZoomStaticTiler(c_slide, 'WSI_tmp', levels, args.base_mag, args.objective, args.format, args.tile_size, args.overlap, True, args.quality, args.workers, args.background_t).run()
        nested_patches(c_slide, out_base, levels, ext=args.format)
        shutil.rmtree('WSI_tmp_files') 
