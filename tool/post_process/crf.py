...
def crf(original_image, annotated_image, prob, use_2d = True):
...
#store_path = '/data1/ian/share_code/crf'
store_path = sys.argv[1]
print(sys.argv[2])
prob = int(sys.argv[2])/50
...
for f in tqdm.tqdm(files):
    if  not ('probas' in f):
        ...
        y = cv2.imread(mask)
        WSI = os.path.join(WSI_path,f[:-4] + '.png')
        store = os.path.join(store_path, f[:-4] + '.png')
        print(WSI)
        X=cv2.imread(WSI)
        x_shape = X.shape
        y_shape = y.shape
        ...
        ret, y = cv2.threshold(y, 127, 255, cv2.THRESH_BINARY)
        crfimage = crf(X,y,prob)
        crfimage = cv2.resize(crfimage,[y_shape[1], y_shape[0]], interpolation=cv2.INTER_AREA)
