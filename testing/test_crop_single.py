...
if __name__ == '__main__':
    ...
    elif args.dataset == 'cts':
        folder = ('test-cts')
        path_base = os.path.join( folder, 'input')
        out_base = os.path.join( folder, 'patches')
        thumbnails = os.path.join( folder, 'thumbnails')
        makedirs(thumbnails, exist_ok=True)
        
    all_slides = glob.glob(join(path_base, '*.svs')) + glob.glob(join(path_base, '*.tif')) + glob.glob(join(path_base, '*.jpg'))
