...
def train(train_df, milnet, criterion, optimizer, args):
    ...
    for i in range(len(train_df)):
        ...
        _, ins_prediction = milnet(bag_feats)
        sorted_prediction, _ = torch.sort(ins_prediction, 0, descending=True)

        max_prediction = torch.mean(sorted_prediction[:5])
...
def test(test_df, milnet, criterion, optimizer, args):
    ...
    with torch.no_grad():
        for i in range(len(test_df)):
            ...
            test_predictions.extend([(1.0*torch.sigmoid(max_prediction)).squeeze().cpu().numpy()])
...
def main():
    ...
    if args.model == 'dsmil':
        import dsmil_plus_abmil as mil
    ...
    best_score = 0
    save_path = os.path.join('weights_upper_brench', datetime.date.today().strftime("%m%d%Y"))
