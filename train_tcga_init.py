...
def train(train_df, milnet, criterion, optimizer, args):
    ...
    for i in range(len(train_df)):
        ...
        ins_prediction, bag_prediction, _, _, _ = milnet(bag_feats)
        sorted_prediction, _ = torch.sort(ins_prediction, 0, descending=True)

        max_prediction = torch.mean(sorted_prediction[:args.n_critical])
...
def main():
    ...
    parser.add_argument('--n_critical', type=int, default=3)
    ...
    if args.model == 'dsmil':
        import dsmil_vis_critical  as mil
        #import dsmil as mil
    elif args.model == 'abmil':
        import abmil as mil
    
    i_classifier = mil.FCLayer(in_size=args.feats_size, out_size=args.num_classes).cuda()
    b_classifier = mil.BClassifier(input_size=args.feats_size, output_class=args.num_classes, dropout_v=args.dropout_node, nonlinear=args.non_linearity, n_critical=args.n_critical).cuda()
    milnet = mil.MILNet(i_classifier, b_classifier).cuda()
    milnet.init_iclassifier()
    if args.model == 'dsmil':
        #pass
        #state_dict_weights = torch.load('init.pth')
        #milnet.load_state_dict(state_dict_weights, strict=False)
        
        if '16' in args.dataset:
            print('using c16 weight')
            #state_dict_weights = torch.load('init.pth')
            state_dict_weights = torch.load('weights_upper_brench/10302022/1.pth')
            for k, v  in state_dict_weights.items():
                print(k)    


            state_dict_weights["i_classifier.fc.0.weight"] = state_dict_weights["fc.0.weight"]
            state_dict_weights["i_classifier.fc.0.bias"] = state_dict_weights["fc.0.bias"]
    ...
    best_score = 0
    if '16' in args.dataset:
        weight_dir = 'weights_C16'
    elif 'project' in args.dataset:
        weight_dir = 'weights_project'
    else:
        weight_dir = 'weights'
    save_path = os.path.join(weight_dir, datetime.date.today().strftime("%m%d%Y"))
    ...
    for epoch in range(1, args.num_epochs):
        ...
        if abs(current_score-previous_score) < 1 and current_score < 70 and current_score < best_score:
            counter += 1
        else:
            counter = 0

        if counter >= 5:
            print('Reset iclassifier weight')
